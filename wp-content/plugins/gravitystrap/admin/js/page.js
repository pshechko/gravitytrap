jQuery( function($) {
	...
	var newColLi = $('<li class="ui-state-default item-col" type="col"></li>');
	var newCol = jQuery('<ul/>').gravitySortable({
		copy: true,
		dropFromOther: true,
		allowedToDrop: '[type="row"],.input-rule',
		receive: function(e, ui){
			e.stopPropagation();
			console.log('newCol receive');
			if(ui.item.is('[type="row"]')){
				ui.item = newRowLi.clone(true).append(newRow.clone(true));
			}
		}
	});
	...
	var newRow = jQuery('<ul/>').gravitySortable({
		...
		receive: function(e, ui){
			e.stopPropagation();
			console.log('newRow receive');
			ui.item = newColLi.clone(true).append(newCol.clone(true));
		}
	});
